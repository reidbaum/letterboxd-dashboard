...
stats_df = pd.DataFrame({
    'Stat':  ['Films watched', 'Avg rating', 'Longest streak (days)',
              'Longest streak (weeks)', 'Multi-film days'],
    'Value': [f'{films_watched:,}', f'{avg_rating} ★',
              f'{longest_streak} days', f'{longest_week_streak} weeks',
              str(multi_film_days)],
})
fig = go.Figure(go.Table(
    header=dict(values=['Stat', 'Value'],
                fill_color='#1a2230', font=dict(color=LBX_TEXT, size=13),
                align='left', height=36),
    cells=dict(values=[stats_df['Stat'], stats_df['Value']],
               fill_color=LBX_CARD, font=dict(color=LBX_GREEN, size=15),
